...
import { insertJobListingApplication, updateJobListingApplication } from "../db/jobListingsApplication"
...
export async function updateJobListingApplicationStage(
  ...
  if (
    !(await hasOrgUserPermission("applicant_change:job_listing_applications_change_stage"))
  ) {
    return {
      error: true,
...
export async function updateJobListingApplicationRating(
  ...
  if (
    !(await hasOrgUserPermission("applicant_change:job_listing_applicationschange_rating"))
  ) {
    return {
      error: true,

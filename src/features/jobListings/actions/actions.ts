...
import { getNextJobListingStatus } from "../lib/util"
...
export async function createJobListing(
  ...
  if (
    orgId == null ||
    !(await hasOrgUserPermission("applicant_change:job_listings_create"))
  ) {
    return {
      error: true,
...
export async function updateJobListing(
  ...
  if (
    orgId == null ||
    !(await hasOrgUserPermission("applicant_change:job_listings_update"))
  ) {
    return {
      error: true,
...
export async function toggleJobListingStatus(id: string) {
  ...
  if (
    !(await hasOrgUserPermission("applicant_change:job_listings_change_status")) ||
...
export async function deleteJobListing(id: string) {
  ...
  if (!(await hasOrgUserPermission("applicant_change:job_listings_delete"))) {
    return error
  }
